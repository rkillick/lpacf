...
function (x, filter.number = 1, family = "DaubExPhase", smooth.dev = var, 
    AutoReflect = TRUE, tol = 0.1, maxits = 5, plot.it = FALSE, 
    verbose = 0, ReturnAll = FALSE) 
{
    EWS <- ewspec3(x = x, filter.number = filter.number, family = family, 
        smooth.dev = smooth.dev, AutoReflect = AutoReflect, WPsmooth.type = "wavelet")$S
    J <- EWS$nlevels
    Jmax <- J - 1
    Jmin <- round(J/2)
    ...
    fa.ews <- ewspec3(x = Epanechnikov(x)*x, filter.number = filter.number, family = family, 
        smooth.dev = smooth.dev, AutoReflect = AutoReflect, WPsmooth.type = "RM", 
        binwidth = ax)$S
    fb.ews <- ewspec3(x = Epanechnikov(x)*x, filter.number = filter.number, family = family, 
        smooth.dev = smooth.dev, AutoReflect = AutoReflect, WPsmooth.type = "RM", 
        binwidth = bx)$S
    fc.ews <- ewspec3(x = Epanechnikov(x)*x, filter.number = filter.number, family = family, 
        smooth.dev = smooth.dev, AutoReflect = AutoReflect, WPsmooth.type = "RM", 
        binwidth = cx)$S
    f1.ews <- ewspec3(x = Epanechnikov(x)*x, filter.number = filter.number, family = family, 
        smooth.dev = smooth.dev, AutoReflect = AutoReflect, WPsmooth.type = "RM", 
        binwidth = x1)$S
    f2.ews <- ewspec3(x = Epanechnikov(x)*x, filter.number = filter.number, family = family, 
        smooth.dev = smooth.dev, AutoReflect = AutoReflect, WPsmooth.type = "RM", 
        binwidth = x2)$S
    ...
    cnt <- 3
    while ((abs(x3 - x0) > tol * (abs(x1) + abs(x2))) && its <= 
        maxits) {
        if (verbose > 0) {
        ...
        if (f2 < f1) {
            ...
            x2 <- round(R * x1 + C * x3)
            f1 <- f2
            f2.ews <- ewspec3(x = Epanechnikov(x)*x, filter.number = filter.number, 
            ...
            x1 <- round(R * x2 + C * x0)
            f2 <- f1
            f1.ews <- ewspec3(x = Epanechnikov(x)*x, filter.number = filter.number, 
                family = family, smooth.dev = smooth.dev, AutoReflect = AutoReflect, 
    ...
    if (its > maxits) 
        warning(paste("AutoBestBW: exceeded maximum number of iterations: ", 
            maxits))
    if (f1 < f2) 
        ans <- x1
    else ans <- x2
